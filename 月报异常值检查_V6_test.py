...
def tc():
    ...
    for i in tqdm(u_input):
        ...
        def t(cpmc):
            ...
            if sjg_all.shape[0] < 5 or abs(stats.skew(sjg.升价格)) <= 0.5:
                return "0|100000|不在抛出逻辑内(样本数量少于5或 偏度小于0.5"
            # diff_median = 0
            # # diff_mean = 0
            else:
                higher = np.quantile(sjg_all.升价格,0.75,interpolation="higher")+sjg_all.升价格.std()
                lower = np.quantile(sjg_all.升价格,0.25,interpolation="higher")-sjg_all.升价格.std()#下四分位数
                
                #变异系数,离散
                cv = sjg_all.升价格.std()/sjg_all.升价格.mean()
                
                higher_q = higher+higher*abs(cv)
                lower_q = lower-higher*abs(cv)

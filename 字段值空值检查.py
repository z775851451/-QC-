...
df = pd.DataFrame(sql_connect(sql = 'select CAST ( 客户名 AS nvarchar ( 500 ) ),CAST ( 品类 AS nvarchar ( 500 ) ),CAST ( 数据库名 AS nvarchar ( 500 ) ),CAST ( 字段名 AS nvarchar ( 500 ) ) from 客户字段及内容_空值'),columns = ['客户名','品类','数据库名','字段名'])
print(df.iloc[0])
...
for i in range(len(df)):
    ...
    sql="select distinct cast(" + a[3]+" as nvarchar(1000)) as "+a[3]+" from "+ a[2]
    # print(sql)
    syntun_cursor = syntun_conn.cursor()
    try:
        syntun_cursor.execute(sql)
    ...
    for jieguo in sqljieguo:
        p = ''
        if jieguo[0] == "#N/A":
            p = [a[2],  str(xcolumns), "列存在#N/A:", jieguo[0]]
        elif jieguo[0] == "0":
            p = [a[2],  str(xcolumns), "列存在0:", jieguo[0]]
        elif jieguo[0] == "null":
            p = [a[2],  str(xcolumns), "列存在null:", jieguo[0]]
        elif jieguo[0] is None:
            p = [a[2],  str(xcolumns), "列存在空值:", jieguo[0]]
        elif jieguo[0].startswith(' '):
            p = [a[2],  str(xcolumns), "列空格开头:", jieguo[0]]
        elif jieguo[0].endswith(' '):
            p = [a[2],  str(xcolumns), "列空格结尾:", jieguo[0]]
        if p != "":
            dd_list.append(p)
        # break

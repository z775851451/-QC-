...
logging.basicConfig(filename='月报检查.log')
def yb():
    ...
    def zlg(i):
        ...
        # 15 京东 2022-08-01
        sql_jd = "select * from [item_temp].[dbo].乳品京东真实url \
            WHERE 月份 >= " + "'" + month[-5]+'01' + "'" + " "
    ...
    for i in tqdm(u_input):
        ...
        try:
            zlg(i)
            print('文件已输出,请到【结果】文件夹下获取')
        except:
            print('--------------------------------------------------------------------------------------------------------------------------------')
            print('当前序号错误Error请查看log,已跳过')
            print('--------------------------------------------------------------------------------------------------------------------------------')

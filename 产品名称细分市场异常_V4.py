...
# '大类','',                                                                                                        '',                                 '',                     '','','                                                                                             ','','',                                                                            '','','','                                                                                                                      ',''
df = pd.DataFrame(sql_connect(sql = 'select CAST ( 大类 AS nvarchar ( 500 ) ),CAST ( 客户 AS nvarchar ( 500 ) ),CAST ( 数据库名 AS nvarchar ( 500 ) ),CAST ( 制造商 AS nvarchar ( 500 ) ),CAST ( 产品名称 AS nvarchar ( 500 ) ),CAST ( 月份 AS nvarchar ( 500 ) ),CAST ( 销售额 AS nvarchar ( 500 ) ),CAST ( 判断字段 AS nvarchar ( 500 ) ),CAST ( 包含内容1 AS nvarchar ( 500 ) ),CAST ( 包含内容2 AS nvarchar ( 500 ) ),CAST ( 不包含内容1 AS nvarchar ( 500 ) ),CAST ( 不包含内容2 AS nvarchar ( 500 ) ),CAST ( 不包含内容3 AS nvarchar ( 500 ) ),CAST ( 字段内容 AS nvarchar ( 500 ) ) from 产品名称判断细分市场异常'),columns = ['大类','客户','数据库名','制造商','产品名称','月份','销售额','判断字段','包含内容1','包含内容2','不包含内容1','不包含内容2','不包含内容3','字段内容'])

#包含内容1的空值作为反查标识,但为空将报错,所以先替换为一二三
    ...
    for i in range(len(gz_df)):
        ...
        if '一二三' not in set(bhnr_1):
            if k not in (['[item].[dbo].yili_593','send_out.dbo.YILI_DATA_593_E_MONTH_NEW']):
                for i in set(bhnr_1):
                    lg_df[cpmc].map(lambda x:ts_li.append(x) if i in x[0:x.find('+')] else x)
            else:
                for i in set(bhnr_1):
...
with pd.ExcelWriter(f'结果/细分市场规则表_抛出结果.xlsx') as mn_writer:
    df.to_excel(mn_writer,sheet_name='规则表',na_rep='',index=False)
    yc_data.to_excel(mn_writer,sheet_name='抛出',na_rep='',index=False)



print(yc_data['字段内容'].value_counts())
